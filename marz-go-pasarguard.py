...
Version: 2.0.2 (Multi-DB Support - Indentation Fix)
...
def get_db_config(env_path: str, name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"{CYAN}=== {name.toUpperCase()} DATABASE SETTINGS ({config['db_type']}) ==={RESET}")
...
# ==========================================================
#                  FIXED FUNCTION (Indentation Corrected)
# ==========================================================
...
def check_dependencies():
    ...
    if not PSYCOPG2_AVAILABLE:
        python_deps.append("psycopg2-binary") # فقط در صورت نیاز نصب کن
...
def display_menu():
    ...
    print(f"┃{YELLOW}              v2.0.2 (Fix)               {CYAN}┃")
    print(f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛{RESET}")
    print()
    print("Menu:")
...
# --- Migration Functions (بازنویسی شده با SQL پویا) ---
...
def migrate_admins(marzban_conn, pasarguard_conn, pasarguard_db_type):
    ...
    with pasarguard_conn.cursor() as cur:
        ...
        cur.execute(get_table_exists_sql(pasarguard_db_type, "admins"))
        if cur.fetchone() is None:
            # Create admins table (Data types are mostly compatible)
            cur.execute("""
                CREATE TABLE admins (
                    id INT PRIMARY KEY,
                    username VARCHAR(255) NOT NULL,
        ...
        for a in admins:
            ...
            # For ON DUPLICATE KEY UPDATE, values are repeated
            if pasarguard_db_type == "mysql":
                update_values = (
                    a["username"], a["hashed_password"],
                    a["created_at"], a["is_sudo"], a["password_reset_at"],
...
def ensure_default_core_config(pasarguard_conn, pasarguard_db_type):
    ...
    with pasarguard_conn.cursor() as cur:
        cur.execute(get_table_exists_sql(pasarguard_db_type, "core_configs"))
        if cur.fetchone() is None:
            # Note: JSON type is available in both modern MySQL and Postgres
            cur.execute("""
                CREATE TABLE core_configs (
                    id INT PRIMARY KEY,
                    created_at TIMESTAMP NOT NULL,
        ...
        if cur.fetchone()["cnt"] == 0:
            ...
            # Use NOW() for MySQL, CURRENT_TIMESTAMP for Postgres (NOW() works for both)
            cur.execute(
                """
                INSERT INTO core_configs
                (id, created_at, name, config, exclude_inbound_tags, fallbacks_inbound_tags)
...
def migrate_xray_config(pasarguard_conn, pasarguard_db_type, xray_config):
    ...
    with pasarguard_conn.cursor() as cur:
        ...
        existing = cur.fetchone()
        if existing:
            # Note: config data might be dict or str, handle access safely
            config_data = existing.get("config") or existing[3] # Access by index if name fails
            cur.execute(
                """
                INSERT INTO core_configs
                (id, created_at, name, config, exclude_inbound_tags, fallbacks_inbound_tags)
        ...
        columns = ["id", "created_at", "name", "config", "exclude_inbound_tags", "fallbacks_inbound_tags"]
        # Custom logic because we want to update created_at = NOW()
        if pasarguard_db_type == "mysql":
            sql = """
                INSERT INTO core_configs (id, created_at, name, config, exclude_inbound_tags, fallbacks_inbound_tags)
                VALUES (%s, NOW(), %s, %s, '', '')
...
def migrate_inbounds_and_associate(marzban_conn, pasarguard_conn, pasarguard_db_type):
    ...
    with pasarguard_conn.cursor() as cur:
        ...
        if cur.fetchone() is None:
            ...
            """)
            # Note: Skipping FOREIGN KEY constraints for wider compatibility/simplicity
            print(f"{GREEN}Created inbounds_groups_association table in Pasarguard ✓{RESET}")
            time.sleep(0.5)
...
def check_dependencies():
    ...
    # Check Python packages
    python_deps = ["pymysql", "python-dotenv", "psycopg2-binary"] # افزوده شد: psycopg2-binary
    missing_pip_deps = []
    for pkg in python_deps:
        if subprocess.run(f"pip3 show {pkg}", shell=True, capture_output=True).returncode != 0:
            missing_pip_deps.append(pkg)
...
def display_menu():
    ...
    print(f"┃{YELLOW}          Power By: ASiSSK               {CYAN}┃")
    print(f"┃{YELLOW}      Marz ➜ Pasarguard (Multi-DB)       {CYAN}┃")
    print(f"┃{YELLOW}              v2.0.1 (Fix)               {CYAN}┃")
    print(f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛{RESET}")
    print()
    print("Menu:")
    print("1. Change Database and phpMyAdmin Ports")
...
def change_db_port():
    ...
    try:
        ...
        if os.path.exists(env_file):
            ...
            # به‌روزرسانی SQLALCHEMY_DATABASE_URL (agnostic regex)
            # This regex finds :<port>/<db_name>" and replaces the port
            content = re.sub(
                r'(@[^:]+:)\d+(/[^\"]+")',
                rf'\g<1>{db_port}\2',
                content
        ...
        # Update docker-compose.yml (This part is tricky and assumes your structure)
        compose_file = DOCKER_COMPOSE_FILE_PATH
        if os.path.exists(compose_file):
            with open(compose_file, 'r', encoding='utf-8') as file:
                content = file.read()
            
            # Update database command port (MySQL/MariaDB)
            if re.search(r'--port=\d+', content):
                content = re.sub(r'--port=\d+', f'--port={db_port}', content)
            
            # Update database ports mapping (PostgreSQL)
            # Example: "5432:5432" -> "NEW_PORT:5432"
            content = re.sub(r'ports:\s*- "\d+:5432"', f'ports:\n      - "{db_port}:5432"', content)
            
            # Update PMA_PORT (for phpMyAdmin/pgAdmin)
            if re.search(r'PMA_PORT: \d+', content):
                content = re.sub(r'PMA_PORT: \d+', f'PMA_PORT: {db_port}', content)
            
            # Update APACHE_PORT (for phpMyAdmin)
            if re.search(r'APACHE_PORT: \d+', content):
                content = re.sub(r'APACHE_PORT: \d+', f'APACHE_PORT: {apache_port}', content)
...
if __name__ == "__main__":
    ...
    # افزوده شد: اطمینان از ایمپورت شدن psycopg2.extras
    try:
        from psycopg2.extras import DictCursor
    except ImportError:
        # check_dependencies باید این را مدیریت کرده باشد
        if 'psycopg2' not in sys.modules:
             print(f"{RED}Warning: psycopg2 driver not found, PostgreSQL will not work.{RESET}")
        else:
             print(f"{RED}Warning: psycopg2.extras not found, PostgreSQL might not work correctly.{RESET}")
        pass
...
def connect(cfg: Dict[str, Any]):
    ...
    try:
        if db_type == "mysql":
            ...
            conn = psycopg2.connect(**conn_args)
            # استفاده از DictCursor برای PostgreSQL
            conn.cursor_factory = psycopg2.extras.DictCursor
        else:
            raise ValueError(f"Unsupported db_type: {db_type}")
...
if __name__ == "__main__":
    if sys.version_info < (3, 6):
        print("Python 3.6+ required.")
        sys.exit(1)
    # افزوده شد: اطمینان از نصب psycopg2.extras
    try:
        from psycopg2.extras import DictCursor
    except ImportError:
        pass # check_dependencies باید این را مدیریت کرده باشد
