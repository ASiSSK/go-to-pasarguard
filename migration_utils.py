...
def change_db_port():
        ...
        if os.path.exists(env_file):
            ...
            for line in lines:
                if line.startswith('DB_PORT='):
                    ...
                    port_updated = True
                elif line.startswith('SQLALCHEMY_DATABASE_URL='):
                    new_lines.append(re.sub(r':\d+/', f':{port}/', line))
                    url_updated = True
                else:
            ...
            if not port_updated:
                new_lines.append(f'DB_PORT={port}\n')
            if not url_updated:
                new_lines.append(f'SQLALCHEMY_DATABASE_URL="mysql+asyncmy://user:password@127.0.0.1:{port}/pasarguard"\n')
    ...
    inputokay = input("Press Enter to return to the menu...")
    return success
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            inputokay = input("Press Enter to continue...")
